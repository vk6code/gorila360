    ...
    <div class="px-4 pt-6 pb-3 sticky top-0 bg-[#0E0E0E] z-10 border-b border-[#333333]">
      ...
      <div class="flex items-center justify-between gap-2 overflow-x-auto pb-2 scrollbar-hide">
        <div
          ...
          class="flex flex-col items-center gap-1 min-w-[60px] cursor-move active:scale-95 transition-transform"
        >
    ...
    <div class="flex-1 px-4 py-4">
      <div class="grid grid-cols-7 gap-1.5">
        ...
        <div
          ...
          :class="[
            'rounded-xl bg-[#141414] border border-[#333333] p-1.5 flex flex-col justify-between transition-all aspect-square relative',
...
const onDragStart = (event, type) => {
  ...
  // Add a ghost image if needed, or browser default
};

const { mutate: updatePlan } = useMutation(UPDATE_DAILY_PLAN);

const onDrop = async (event, day) => {
  isDragOver.value = null;
  const typeCode = event.dataTransfer.getData('dayType');

  if (typeCode && day.date && !day.date.startsWith('placeholder')) {
    // Optimistic Update
    localUpdates.value = {
      ...localUpdates.value,
      [day.date]: typeCode
    ...
    try {
      ...
      // Ideally refetch or just rely on optimistic update until page reload
    } catch (e) {
      console.error('Failed to update plan:', e);
      // Revert optimistic update
      const updates = { ...localUpdates.value };
      delete updates[day.date];
      localUpdates.value = updates;
    }
  }
};
</script>
